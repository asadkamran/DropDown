...
public final class MaterialDropDown: UIView {
	...
	public static weak var VisibleDropDown: MaterialDropDown?
	...
	*/
	public var animationEntranceOptions: UIView.AnimationOptions = MaterialDropDown.animationEntranceOptions
	
	/**
	The option of the hide animation. Only change the caller. To change all drop down's use the static var.
	*/
	public var animationExitOptions: UIView.AnimationOptions = MaterialDropDown.animationExitOptions
...
private extension MaterialDropDown {
...
extension MaterialDropDown {
	...
	*/
    ...
    public func show(onTopOf window: UIWindow? = nil, beforeTransform transform: CGAffineTransform? = nil, anchorPoint: CGPoint? = nil) -> (canBeDisplayed: Bool, offscreenHeight: CGFloat?) {
		if self == MaterialDropDown.VisibleDropDown && MaterialDropDown.VisibleDropDown?.isHidden == false { // added condition - DropDown.VisibleDropDown?.isHidden == false -> to resolve forever hiding dropdown issue when continuous taping on button - Kartik Patel - 2016-12-29
			return (true, 0)
		}

		if let visibleDropDown = MaterialDropDown.VisibleDropDown {
			visibleDropDown.cancel()
		}

		willShowAction?()

		MaterialDropDown.VisibleDropDown = self
	...
	/// Hides the drop down.
	public func hide() {
		if self == MaterialDropDown.VisibleDropDown {
			/*
			...
			*/
			MaterialDropDown.VisibleDropDown = nil
		}
...
extension MaterialDropDown: UITableViewDataSource, UITableViewDelegate {
